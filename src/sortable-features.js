...
const { BaseControl, TextControl, TextareaControl, Button, ButtonGroup } =
	wp.components;
				...
				{position === clickedItem && (
					<Fragment>
						<TextControl
							...
							value={feature.title}
							onChange={(value) => onFeatureChange("title", value, position)}
						/>
						<TextareaControl
							label={__("Content", "essential-blocks")}
							value={feature.content}
							onChange={(value) => onFeatureChange("content", value, position)}
						/>
						<BaseControl label={__("Icon Type", "essential-blocks")}>
							<ButtonGroup className="eb-featurelist-icon-type">
								{MEDIA_TYPES.map((item) => (
									<Button
										isLarge
										isPrimary={feature.iconType === item.value}
										isSecondary={feature.iconType !== item.value}
										onClick={() =>
											onFeatureChange("iconType", item.value, position)
										}
									>
										{item.label}
									</Button>
						...
						{feature.iconType === "image" && !feature.featureImage && (
							<MediaUpload
								// onSelect={
								// 	(({ id, url }) =>
								...
								onSelect={({ id, url }) => {
									onFeatureChange("featureImageId", id, position);
									onFeatureChange("featureImage", url, position);
								}}
								type="image"
						...
						{feature.iconType === "image" && feature.featureImage && (
							<ImageAvatar
								...
								onDeleteImage={() => {
									onFeatureChange("featureImageId", null, position);
									onFeatureChange("featureImage", null, position);
								}}
							/>
						)}
						{feature.iconType === "icon" && feature.icon && (
							<ColorControl
								label={__("Icon Color", "essentail-blocks")}
								color={feature.iconColor}
								onChange={(value) =>
									onFeatureChange("iconColor", value, position)
								}
							/>
						)}
						<ColorControl
							label={__("Background Color", "essentail-blocks")}
							color={feature.iconBackgroundColor}
							onChange={(value) =>
								onFeatureChange("iconBackgroundColor", value, position)
							}
						/>
						<TextControl
